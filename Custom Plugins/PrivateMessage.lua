		...
		do
			...
			spawn(function()
				...
				notify.Clicked:connect(function() 
					local response = SyncAPI.DisplayQuestionMessage(p,"PM from " .. user.Name,message,"Exclamation",options)
					if (response == "Reply") then
						local btn,msg = SyncAPI.DisplayTextInput(player,"Reply to " .. p.Name,"Type a reply to " .. p.Name,"Check",roptions)
						if (btn == "Send") then
